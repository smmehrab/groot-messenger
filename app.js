...
signOutBtn.addEventListener('click', (e) => {
    ...
    rdb.ref('.info/connected').once('value').then((snapshot) => {
        if (snapshot.val()) {
            ...
            console.log('Ensure Internet Connection to Sign Out');
        }
    });
});
...
function displayPeople() {
    ...
    incomingRequests.get().then((snapshotIn) => {
        ...
        snapshotIn.docs.map(doc => doc.data()).forEach(function (doc) {
        ...
        outgoingRequests.get().then((snapshotOut) => {
            outgoingRequestList = [];
            snapshotOut.docs.map(doc => doc.data()).forEach(function (doc) {
                users.doc(doc.receiver).get().then(function (user) {
                    renderPeople("outgoing", user.data().email);
                    outgoingRequestList.push(user.data().email);
                });
            });

            users.get().then((snapshotPeople) => {
                peopleList = [];
                snapshotPeople.docs.map(doc => doc.data()).forEach(function (doc) {
                    if (doc.email != auth.currentUser.email && !incomingRequestList.includes(doc.email) && !outgoingRequestList.includes(doc.email)) {
                        renderPeople("general", doc.email);
                        peopleList.push(doc.email);
                    }
                });

                console.log(incomingRequestList);
                console.log(outgoingRequestList);
...
function renderPeople(type, email) {
    ...
    document.getElementById('peopleDiv').appendChild(peopleItem);

}

function addAsFriend(status, email, addFriend) {
    addFriend.classList.add('disableClick');
    ...
    query.get().then((snapshot) => {
        ...
        if (status == "send") {
            ...
            addRequest({ "sender": user, "receiver": remotePerson }).then(function (doc) {
                ...
                console.log(doc);
                addFriend.classList.remove('disableClick');
                addFriend.innerHTML = "";
                addFriend.appendChild(document.createTextNode("person_add_disabled"));
...
function handleFriendRequest(status, email, addFriend, rejectFriend) {
    addFriend.classList.add('disableClick');
    rejectFriend.classList.add('disableClick');
    ...
    query.get().then((snapshot) => {
        ...
        if (status == "accept") {
            ...
            acceptRequest({ "sender": remotePerson, "receiver": user }).then(function (doc) {
                ...
                console.log(doc);
                addFriend.classList.remove('disableClick');
                rejectFriend.classList.remove('disableClick');
                addFriend.innerHTML = "";
...
function chatWithFriend(friend, messageFriend) {
    messageFriend.classList.add('disableClick');
